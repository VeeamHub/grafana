#!/bin/bash
##      .SYNOPSIS
##      Grafana Dashboard for Veeam Backup for Microsoft Office 365 v2.0 - Using RestAPI to InfluxDB Script
## 
##      .DESCRIPTION
...
veeamBearer=$(curl -X POST --header "Content-Type: application/x-www-form-urlencoded" --header "Accept: application/json" -d "grant_type=password&username=$veeamUsername&password=$veeamPassword&refresh_token=%27%27" "$veeamRestServer:$veeamRestPort/v2/token" -k --silent | jq -r '.access_token')
...
##
veeamVBOUrl="$veeamRestServer:$veeamRestPort/v2/BackupRepositories"
...
##
veeamVBOUrl="$veeamRestServer:$veeamRestPort/v2/Proxies"
...
##
veeamVBOUrl="$veeamRestServer:$veeamRestPort/v2/Jobs"
...
for id in $(echo "$veeamJobsUrl" | jq -r '.[].id'); do
    ...
    # Backup Job Sessions
    veeamVBOUrl="$veeamRestServer:$veeamRestPort/v2/Jobs/$idJob/jobsessions"
    ...
    for id in $(echo "$veeamJobSessionsUrl" | jq -r '.[].id'); do
      ...
      case $readRateUnit in
        ...
        ;;
        KB/s)
            readRateNum=$(echo "scale=4; $readRateNum / 1000" | bc -l)
        ;;
        MB/s)
        ;;
        esac
      ...
      case $writeRateUnit in
        ...
        ;;
        KB/s)
            writeRateNum=$(echo "scale=4; $writeRateNum / 1000" | bc -l)
        ;;
        MB/s)
        ;;
        esac 
      ...
      case $transferredDataUnit in
        ...
        ;;
        KB)
            transferredDataNum=$(echo "scale=4; $transferredDataNum / 1000" | bc -l)
        ;;
        MB)
        ;;
        esac 
...
##
veeamVBOUrl="$veeamRestServer:$veeamRestPort/v2/RestoreSessions"
